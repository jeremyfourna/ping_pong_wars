...
Meteor.methods({
	addAGame: function(gameData) {
		...
		var pointBase = 40;
		var winner = function() {
			if (gameData.scorePlayer1 > gameData.scorePlayer2) {
		...
		if (winner() === gameData.player1) {
			...
			pointsToLoose = Math.round(lastPoints2 - pointBase * (1 - 1 / (1 + Math.pow(10, (lastPoints2 - lastPoints1) / 400))));
			...
			pointsToLoose = Math.round(lastPoints1 - pointBase * (1 - 1 / (1 + Math.pow(10, (lastPoints1 - lastPoints2) / 400))));
			Meteor.users.update({ _id: gameData.player2 }, {
				$push: {
					'profile.points': pointsToAdd
				}

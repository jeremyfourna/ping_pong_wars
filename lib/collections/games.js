		...
		for (var i = 0; i < allGamesLength; i++) {
			...
			if (kEqualForBothPlayers(pointBasePlayer1, pointBasePlayer2)) {
				if (winner(allGames[i]) === allGames[i].player1) {
					...
					pointsToAdd = Math.round(pointBasePlayer1 * (1 / (1 + Math.pow(10, pointsDifference(lastPointsPlayer2, lastPointsPlayer1) / 400))));
					Meteor.users.update({ _id: allGames[i].player1 }, {
						$push: {
							'profile.points': lastPointsPlayer1 - pointsToAdd
						}
				...
				if (winner(allGames[i]) === allGames[i].player1) {
					...
					pointsToAdd = Math.round(pointBasePlayer2 * (1 / (1 + Math.pow(10, pointsDifference(lastPointsPlayer2, lastPointsPlayer1) / 400))));
